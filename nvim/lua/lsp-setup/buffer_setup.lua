...
local function setup_lsp_keymaps(_, bufnr)
    ...
    utils.map("n", "<m-cr>", fzf.lsp_code_actions, {silent = true, buffer = bufnr})
...
local function setup_diagnostics_keymaps(_, bufnr)
    utils.map('n', '[e', function() vim.diagnostic.goto_prev({ float = true }) end, {silent = true, buffer = bufnr})
    utils.map('n', ']e', function() vim.diagnostic.goto_next({ float = true }) end, {silent = true, buffer = bufnr})
    utils.map('n', '<leader>e', function() lsp_utils.toggle_diagnostics_list(false) end, {silent = true, buffer = bufnr})
    utils.map('n', '<leader>E', function() lsp_utils.toggle_diagnostics_list(true) end, {silent = true, buffer = bufnr})
end

-- setup formatting keymaps
local function setup_formatting_keymaps(client, bufnr)
    if client.server_capabilities.documentFormattingProvider then
        utils.map('n', '<c-f>', function() vim.lsp.buf.format({async=true}) end, {silent = true, buffer = bufnr})
    end

    if client.server_capabilities.documentRangeFormattingProvider then
        utils.map('v', '<c-f>', function() vim.lsp.buf.format({async=true}) end, {silent = true, buffer = bufnr})
    end
end
